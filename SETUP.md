...
- **셀원 명단 동시 편집**: `set_members`는 명단 전체를 교체(last-writer-wins)합니다. 두 사람이 동시에 편집하면 한쪽이 덮어써질 수 있습니다. → 앱이 **저장 직전 최신 명단을 재확인(OCC)** 하여, 그 사이 바뀌었으면 덮어쓰지 않고 최신을 불러와 다시 확인하도록 경고합니다(무음 손실 방지). 완전한 동시성(버전 필드/병합)은 미도입.
- **멤버 식별**: 각 셀원은 숨은 고유 `id`를 가지며 이름은 표시·수정용. 이름을 바꿔도 주문/호스트 링크가 유지됩니다. 기존(레거시) 문자열 멤버는 `id=이름`으로 호환되며, 셀원을 처음 편집할 때 객체형으로 자동 전환됩니다.
- **세션은 '당일(KST)' 기준**: `get_room_day(today)`만 조회하므로, 자정을 넘기면 전날 열어둔 세션·주문은 화면에서 보이지 않습니다. **마감은 자정 전에** 하는 것을 권장합니다(다중 날짜 세션 미지원).
- **링크 = 권한**: 셀 링크(room_id)를 아는 사람은 그 셀의 주문/셀원 편집이 가능합니다. 링크는 비공개 채널로만 공유하세요. (단, 셀원 추가/이름변경/삭제는 §7 감사 로그에 남습니다.)

## 7. 셀원 수정 감사 로그 (`member_logs`)
누가 어떤 셀원을 추가/이름변경/삭제했는지 기록합니다. 무로그인 모델이라 "누가"는 **편집자가 그 셀에서 고른 본인 이름**(일반) 또는 **`관리자`**(IT)로 자기신고됩니다.
