...
- 실제 구성원 이름은 코드에 넣지 않고 Supabase `cells` 테이블에만 저장합니다.
...
별도 운영 배포에 값을 직접 넣는 경우에도 아래처럼 실제 구성원 명단이나 주문 데이터는 코드에 넣지 않습니다.
...
관리자 화면의 **`🧹 섬김정리`**에서 진행 중인 주문을 마감하려면 최신 `db-migration-admin-tools.sql`을 SQL Editor에서 실행합니다. 진행 중인 섬김에는 **`주문 마감`** 버튼이 표시됩니다.
...
### 대시보드 (사용 현황 · 출석)
관리자 화면 상단 **`📊 대시보드`** 버튼에서 셀별 현황을 봅니다.

- **설치**: `db-migration-admin-dashboard.sql`을 SQL Editor에서 Run. `get_admin_dashboard(p_days)` 집계 RPC가 생성됩니다(스키마 변경 없음, 재실행 안전). 이 RPC는 `authenticated`(관리자)에게만 execute 부여 + 내부에서 `auth.role()` 재확인.
- **① 셀별 사용 빈도**: 최근 N일(7/30 토글, KST)간 셀별 **섬김 오픈/마감 수**를 날짜별 막대로 표시(색: 마감=브랜드/진행=액센트). `sessions`에서 `state in ('open','closed')`인 행을 날짜별로 집계.
- **② 셀별 구성원 출석**: 구성원이 이 앱에서 **주문 또는 '안 마심'으로 의사표현한 날 수**를 출석으로 인정. 최근 N일 출석 / 전체 누적 출석을 함께 표시. `orders`에서 `member_id`별 `count(distinct date)`로 집계(오늘 포함).
- **집계 대상 기간**: 출석 누적은 `orders`에 남아 있는 전체 기간 기준. 주문은 KST 기준 최근 2일까지만 기록 가능하지만 과거 행은 보존되므로 누적 집계에 반영됩니다.
...
## 6. 알려진 한계 (신뢰그룹 전제 — QA에서 완화)
소규모 신뢰 그룹(셀)을 전제로 한 설계라 아래는 의도된 한계입니다. 앱에서 가볍게 완화돼 있습니다.

- **구성원 명단 동시 편집**: `set_members`는 명단 전체를 교체(last-writer-wins)합니다. 두 사람이 동시에 편집하면 한쪽이 덮어써질 수 있습니다. → 앱이 **저장 직전 최신 명단을 재확인(OCC)** 하여, 그 사이 바뀌었으면 덮어쓰지 않고 최신을 불러와 다시 확인하도록 경고합니다(무음 손실 방지). 완전한 동시성(버전 필드/병합)은 미도입.
- **멤버 식별**: 각 구성원은 숨은 고유 `id`를 가지며 이름은 표시·수정용. 이름을 바꿔도 주문/호스트 링크가 유지됩니다. 기존(레거시) 문자열 멤버는 `id=이름`으로 호환되며, 구성원을 처음 편집할 때 객체형으로 자동 전환됩니다.
- **세션은 '당일(KST)' 기준**: `get_room_day(today)`만 조회하므로, 자정을 넘기면 전날 열어둔 세션·주문은 화면에서 보이지 않습니다. **마감은 자정 전에** 하는 것을 권장합니다(다중 날짜 세션 미지원).
- **링크 = 권한**: 셀 링크(room_id)를 아는 사람은 그 셀의 주문/구성원 편집이 가능합니다. 링크는 비공개 채널로만 공유하세요. (단, 구성원 추가/이름변경/삭제는 §7 감사 로그에 남습니다.)

## 7. 구성원 수정 감사 로그 (`member_logs`)
누가 어떤 구성원을 추가/이름변경/삭제했는지 기록합니다. 무로그인 모델이라 "누가"는 **편집자가 그 셀에서 고른 본인 이름**(일반) 또는 **`관리자`**(IT)로 자기신고됩니다.

- **설치**: `db-migration-member-logs.sql`을 SQL Editor에서 Run(또는 `db-setup.sql` 전체 재실행). `member_logs` 테이블 + `member_diff()` + `set_members(p_id, p_members, p_actor)` 3-인자 버전이 생성됩니다.
- **기록 방식**: `set_members` 내부에서 저장 직전 명단과 새 명단을 **숨은 id 기준으로 diff** 하여, 변경분이 있을 때만 `member_logs`에 1행 기록(순서만 바뀐 재저장은 기록 안 함). 정의자(SECURITY DEFINER) 권한으로 insert 하므로 anon은 로그를 직접 쓰거나 읽을 수 없습니다.
- **열람**: 관리자 화면(`?admin`) → 각 셀 행의 **`🕘 내역`** 버튼 → 최신순 100건. 추가(초록)/이름변경(파랑)/삭제(빨강)로 표시. 읽기는 `authenticated`(관리자)만 RLS로 허용.
- **한계**: 자기신고 actor라 신원 위조 가능(신뢰그룹 전제). 로그 자동 삭제(보존주기) 미도입 — 필요 시 주기적 정리.
